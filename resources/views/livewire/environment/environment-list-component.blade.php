    ...
    <div class="table-settings mb-4">
        <div class="row justify-content-between align-items-center">
            <div class="col-9 col-lg-8 d-md-flex">
                ...
                <select wire:model="isActive" class="form-select fmxw-200 d-none d-md-inline"
            ...
            <tbody>
                ...
                @forelse($environments as $environment)
                    <tr>
                        ...
                        <td><span
                                class="fw-normal">{{ $environment->user->first_name . '-' . $environment->user->last_name }}</span>
                        </td>
                        <td><span
                                class="fw-normal d-flex align-items-center">{{ $environment->created_at->diffForHumans() }}</span>
                        </td>
                        <td><span
                                class="fw-normal text-{{ $environment->is_active ? 'success' : 'error' }}">{{ $environment->is_active ? 'Active' : 'In-Active' }}</span>
                        </td>
                        <td>
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1">
                                    <a class="dropdown-item d-flex align-items-center"
                                        href="{{ route('environment.show', [$environment->id]) }}">
                                        <span class="fas fa-user-shield me-2"></span>
                                        View Details
                                    </a>
                                    @if (auth()->user()->is_admin ||auth()->user()->id === $environment->user_id )
                                        <a wire:click="deleteEnvironment({{$environment->id}})" class="dropdown-item text-danger d-flex align-items-center" href="#">
